...
function renderCart(userId, firstname, lastname, cartInfo, deliveryFeeSplit, cartSubtotal, confirmed){
    ...
    summaryDiv.innerHTML = `
        ...
        <button class="cartConfirmButton layer2container buttonText ${confirmed ? "confirmed" : ""} confirmButton" disabled>
    ...
    for (let i = 0; i < cartInfo.length; i++) {
        ...
        itemDiv.innerHTML = `
              ...
              <label>Quantity: <input type="number" class="itemQuantity layer3container" name="quantity" min="1" max="99" value="${item.quantity}" disabled data-itemid="${key}"></label>
        ...
        quantityInput.addEventListener("change", async (event) => {
            ...
            newQuantity = await sendAPIRequest("updateQuantity", {userId: userId, itemId: item._id, newQuantity: newQuantity});
    ...
    <div class="personalCartCheckout">
        ...
        <div class="personalTotal personalCartTotalItem">
            ...
            <span class="personalTotalResult">£${cartSubtotal+deliveryFeeSplit}</span>
        </div>
    </div>
  `;
...
function modifySelectedCart(userId){
    ...
    for (let i = 0; i < personalCarts.length; i++){
        ...
        const summaryDiv = personalCart.getElementsByClassName("personalCartSummary")[0];
        const dropdownDiv = personalCart.getElementsByClassName("personalCartDropdown")[0];
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    if (carts.length === 0) {
        ...
        carts.forEach(cart => {
            ...
            renderCart(cart.userid, cart.firstname, cart.lastname, cart.cart, deliveryFeeSplit, subtotal, cart.confirmed);
        });
